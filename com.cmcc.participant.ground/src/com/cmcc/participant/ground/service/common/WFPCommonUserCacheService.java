 ...
 * 
 */
public class WFPCommonUserCacheService implements IWFPUserService {

    private static WFMessageLog messageLogger = WFMessageLog.getWFMessageLogger(WFPCommonUserCacheService.class);
    ...
    private static final Logger logger = TraceLoggerFactory.getContributionTraceLogger("com.cmcc.participant.ground",
            WFPCommonUserCacheService.class);

    private static WFPCommonUserCacheService userCacheService = new WFPCommonUserCacheService();
     ...
     * 构造方法,初始化缓存对象
     */
    private WFPCommonUserCacheService() {
        try {
            // step1 : 构造方法中初始化缓存对象
    ...
    public static IWFPUserService getInstance() {
        ...
        if (!cacheSwitch) {
            ...
            return OMServiceFactory.getUserService(EngineContextManager.current().getTenantIDSilence());
        }
    ...
    public static void clearCache() {
        ...
        messageLogger.log(LogService.LEVEL_ERROR, "Cache Cleared: ip=" + ip + "size=" + wfpUserCache.size()
                + "　name=wfpUserCache");
    }
    ...
    public WFParticipant getParticipantByID(String userId) throws Exception {
        ...
        if (userParticipant != null) {
            ...
            synchronized (wfpUserCacheLock) {
                wfpUserServiceBean = OMServiceFactory.getUserService(EngineContextManager.current()
                        .getTenantIDSilence());
                // BeanFactory.newInstance().getBean("WFPUserServiceBean");
                // step3 : 如果缓存没有，则查库
    ...
    public List<WFParticipant> getParticipantByOrgId(String orgCode) throws Exception {
        // step1 : 从缓存中获取数据
        List<WFParticipant> participantList = (List<WFParticipant>) wfpUserCache.get(CACHE_KEYHEAD_USERPATISBYORGID
                + orgCode);
        // step2 : 如果缓存中有数据，则直接返回
    ...
    public List<WFParticipant> getAllUserParticipantList() throws Exception {
        ...
        // // step1 : 从缓存中获取数据
        // List<WFParticipant> participantList = (List<WFParticipant>)
        // wfpUserCache.get(CACHE_KEY_ALLUSERPARTIS);
        //
        // // step2 : 如果缓存中有数据，则直接返回
        // if (participantList != null && participantList.size() > 0) {
        // return participantList;
        ...
        // // step3 : 如果缓存没有，则查库
        // participantList =
        // this.wfpUserServiceBean.getAllUserParticipantList();
        //
        // // step4 : 将查询出的结果放入缓存，并返回
