 ...
 * 
 */
public class WFPCommonOrgCacheService implements IWFPOrgService {

    private static final Logger logger = TraceLoggerFactory.getContributionTraceLogger("com.cmcc.participant.ground",
            WFPCommonOrgCacheService.class);

    private static WFMessageLog messageLogger = WFMessageLog.getWFMessageLogger(WFPCommonOrgCacheService.class);
    ...
    private static WFPCommonOrgCacheService orgCacheService = new WFPCommonOrgCacheService();
     ...
     * 构造方法,初始化缓存对象
     */
    private WFPCommonOrgCacheService() {
        try {
            // step1 : 构造方法中初始化缓存对象
    ...
    public static IWFPOrgService getInstance() {
        ...
        if (!cacheSwitch) {
            ...
            return OMServiceFactory.getOrgService(tenantID);
        }
    ...
    public static void clearCache() {
        ...
        messageLogger.log(LogService.LEVEL_ERROR, "Cache Cleared: ip = " + ip + " size=" + wfpOrgCache.size()
                + "　name=wfpOrgCache");
    }
    ...
    public WfpAfOrganization getRootOrganization() throws Exception {
        ...
        if (rootOrganization != null) {
            ...
            synchronized (wfpOrgCacheLock) {
                String tenantID = EngineContextManager.current().getTenantIDSilence();
                wfpOrgServiceBean = OMServiceFactory.getOrgService(tenantID);
                // BeanFactory.newInstance().getBean("WFPOrgServiceBean");
                // step3 : 如果缓存没有，则查库
    ...
    public List<WFParticipant> getRoleListByUserId(String participantID) throws Exception {
        // step1 : 从缓存中获取数据
        List<WFParticipant> roleParticipants = (List<WFParticipant>) wfpOrgCache.get(CACHE_KEYHEAD_ROLESBYUID
                + participantID);
        // step2 : 如果缓存中有数据，则直接返回
    ...
    public List<WFParticipant> getRoleParticipantsByTenantID(String tenantID) throws Exception {
        // step1 : 从缓存中获取数据
        List<WFParticipant> roleParticipants = (List<WFParticipant>) wfpOrgCache.get(CACHE_KEYHEAD_TENANTROLE
                + tenantID);
        // step2 : 如果缓存中有数据，则直接返回
        if (roleParticipants != null && roleParticipants.size() > 0) {
            return roleParticipants;
            ...
            synchronized (wfpOrgCacheLock) {
                ...
                wfpOrgServiceBean = OMServiceFactory.getOrgService(tid);
                // wfpOrgServiceBean =
                // BeanFactory.newInstance().getBean("WFPOrgServiceBean");
                // step3 : 如果缓存没有，则查库
    ...
    public List<WFParticipant> getChildOrgParticipantList(String orgCode) throws Exception {
        // step1 : 从缓存中获取数据
        List<WFParticipant> childOrgParticipantList = (List<WFParticipant>) wfpOrgCache.get(CACHE_KEYHEAD_CHILDORGPATIS
                + orgCode);
        // step2 : 如果缓存中有数据，则直接返回
