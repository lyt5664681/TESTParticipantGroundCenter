...
public class OrganizationConsumer implements MessageListener, ServletContextListener {
	private static WFMessageLog messageLogger = WFMessageLog.getWFMessageLogger(OrganizationConsumer.class);
	...
	public static final String TOPIC = "TOPIC_DISTMDMUSERORGANIZATIONSRV_HQ";
	...
	public void onMessage(Message msg) {
		try {
			...
			initOMBean.initOrganizationData();

			if (msgText.equalsIgnoreCase("quit")) {
				messageLogger.log(LogService.LEVEL_ERROR, "************OrganizationConsumer************:do quit");
				synchronized (this) {
					quit = true;
					this.notifyAll();
				}
	...
	public void init(String topicName) throws Throwable {
		try {
			...
			messageLogger.log(LogService.LEVEL_ERROR, "************OrganizationConsumer************:delay end start connection");
			topicConnection.start();
		} catch (Throwable e) {
			e.printStackTrace();
			messageLogger.log(LogService.LEVEL_ERROR, "************OrganizationConsumer************:init exception " + e);
		}
	}
	...
	public void startSubscribe() throws Throwable {
		...
		for (String singleTopic : TOPIC.split(",")) {
			...
			Thread th = new Thread(new Runnable() {
				public void run() {
					try {
						OrganizationConsumer consumer = new OrganizationConsumer();
						...
						synchronized (consumer) {
							while (!consumer.quit) {
								try {
									System.out.println("************OrganizationConsumer************:开始监听");
									messageLogger.log(LogService.LEVEL_ERROR, "************OrganizationConsumer************:start lisenter ");
									consumer.wait();
								} catch (Throwable ie) {
									ie.printStackTrace();
									messageLogger.log(LogService.LEVEL_ERROR, "************OrganizationConsumer************:thread while Exception :" + ie);
								}
							}
						}
						System.out.println("关闭了");
						messageLogger.log(LogService.LEVEL_ERROR, "************OrganizationConsumer************:consumer close");
						consumer.close();
					} catch (Throwable e) {
						e.printStackTrace();
						messageLogger.log(LogService.LEVEL_ERROR, "************OrganizationConsumer************:thread Exception:" + e);
					}
				}
			});
	...
	public void close() throws JMSException {
		try {
			...
			messageLogger.log(LogService.LEVEL_ERROR, "************OrganizationConsumer************:close Exception " + e);
		}
	}

	static {
		try {
			new OrganizationConsumer().startSubscribe();
		} catch (Throwable e) {
			e.printStackTrace();
			messageLogger.log(LogService.LEVEL_ERROR, "************OrganizationConsumer************:static Exception " + e);
		}
